...
class Draw extends PIXI.Graphics {
    constructor(loc, timeStamp, isPolygon) {
        super();
        this._path = [];
        this._polyPath = [];
        this._recoPath = [];
    ...
    addPolyPathPoint(x, y, timeStamp) {
        this._polyPath.push(x);
        this._polyPath.push(y);
        this._timeStamp.push(timeStamp);
    }
    addPathPointLoc(loc, timeStamp) {
        this._path.push(loc);
        this._timeStamp.push(timeStamp);
    }
    addPathPoint(x, y, timeStamp) {
        var loc = {
            x: x,
            y: y
        };
    ...
    drawPoly() {
        this.clear();
        this.beginFill(_1.Bulle.bulleColor);
        this.drawPolygon(this._polyPath);
        this.endFill();
    }
    setRecoPath(path, timeStamp) {
    ...
    static fromPathToPathNumber(path) {
        var pathNum = [];
        for (var i = 0; i < path.length; i++) {
    ...
    static fromPathToPathNumberX(path) {
        var pathNum = [];
        for (var i = 0; i < path.length; i++) {
            pathNum.push(path[i].x);
        }
    ...
    static fromPathToPathNumberY(path) {
        var pathNum = [];
        for (var i = 0; i < path.length; i++) {
            pathNum.push(path[i].y);
        }
