...
function fastFun (){
	if(! fastBool){
		...
		sensorFast.mousedown=sensorFast.touchstart=function(data){
			...
			newFastBulleText=prompt('Text nouvelle bulle')
			if(newFastBulleText){
				updateWindowSize()
				fastPoX=newPosition.x/scaleScene.scale.x-scene.x-(windowW/scaleScene.scale.x-windowW)/2
				fastPoY=newPosition.y/scaleScene.scale.x-scene.y-(windowH/scaleScene.scale.x-windowH)/2
