...
function linkSelection(data){
	...
	if(selectedLink.length>0){
		for(i=0;i<selectedLink[0].length;i++){
			indexLink=sceneLink.getChildIndex(selectedLink[0][i])
...
function motion(bulleX0,bulleY0){
	if(selectedLink.length>0){
		for(i=0;i<selectedLink[0].length;i++){
			bulleX1=Number(stillBubbleArray[i].x);
			bulleY1=Number(stillBubbleArray[i].y);
