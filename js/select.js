    ...
    for (var j = 0; j < rectTestArray.length; j++) {
        ...
        for (i = 0; i < pathX.length; i++) {
            if (pathX[i] != pathX[i + 1]) {
                if (pathY[i] > yOfSmallestX) {
                    if (pathX[i] < pathX[i + 1]) {
                        if (pathX[i] < detectbulleX && detectbulleX < pathX[i + 1]) {
