...
var spriteMove = new PIXI.Sprite.fromImage('./images/MOVE.png');
var nbrDetect = 0
		...
		for(j=0;j<linkArray.length;j++){
			if(multiArray[i][0]==linkArray[j][1]||multiArray[i][0]==linkArray[j][2]){
				...
				if(multiLinkArray.length==0){
					...
					for(k=0;k<multiLinkArray.length;k++){
...
function multiMove(moveX,moveY){
	for(ii=0;ii<multiArray.length;ii++){
		multiArray[ii][0].x=moveX-multiArray[ii][1]
		multiArray[ii][0].y=moveY-multiArray[ii][2]
	}
	for(ii=0;ii<multiLinkArray.length;ii++){
		currentLink=multiLinkArray[ii][0]
		bulleX0=parseInt(multiLinkArray[ii][1].x)
		bulleY0=parseInt(multiLinkArray[ii][1].y)
		bulleX1=parseInt(multiLinkArray[ii][2].x)
		bulleY1=parseInt(multiLinkArray[ii][2].y)
		currentLink.clear()
...
spriteMove.mousedown = spriteMove.touchstart = function(data){
...
spriteMove.mouseup = spriteMove.mouseupoutside = spriteMove.touchend = spriteMove.touchendoutside = function(data){
	this.dragging = false;
	this.data = null;
	if(drawBool){
		drawDown=false
...
spriteMove.mousemove = spriteMove.touchmove = function(data){
	if(this.dragging){
